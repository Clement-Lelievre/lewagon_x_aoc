...
INPUT = """p=<-1027,-979,-188>, v=<7,60,66>, a=<9,1,-7>
...
rows = list(filter(lambda x: x.strip(), INPUT.splitlines()))
particles = {k: list(map(int, NB_PAT.findall(rows[k]))) for k in range(len(rows))}


class Particle:
    """Represents a particle as described by its x, y, z position, velocity and acceleration"""
    ...
    def get_lon_term_dist_to_origin(self, nb_iter: int = 1_000) -> int:
        for _ in range(nb_iter):
            self.update_particle()
        return sum(map(abs, (self.p_x, self.p_y, self.p_z)))

    def __repr__(self) -> str:
        return f"Particle({self.p_x}, {self.p_y}, {self.p_z}, {self.v_x}, {self.v_y}, {self.v_z}, {self.a_x}, {self.a_y}, {self.a_z})"


answer = min(
    particles, key=lambda x: Particle(*particles[x]).get_lon_term_dist_to_origin()
)
print(answer)

# part 2
# now we need to resolve collisions on the fly

remaining_particles = {k: Particle(*particles[k]) for k in particles}
for _ in range(1_000):
    new_pos = defaultdict(int)
    for i in remaining_particles:
        particle = remaining_particles[i]
        particle.update_particle()
        new_pos[(particle.p_x, particle.p_y, particle.p_z)] += 1
    remaining_particles = {
        ind: obj
        for ind, obj in remaining_particles.items()
        if new_pos[(obj.p_x, obj.p_y, obj.p_z)] == 1
    }
