...
INPUT = """[1518-05-11 00:22] falls asleep
...
inp = sorted(e for e in INPUT.split("\n") if e)

# Find the guard that has the most minutes asleep. What minute does that guard spend asleep the most?
# What is the ID of the guard you chose multiplied by the minute you chose?

guard_nb_pat = re.compile(r"#(\d+)")
minutes_pat = re.compile(r"(\d+)\]")

shifts = [i for i, j in enumerate(inp) if "#" in j]
split_shifts = [
    inp[shifts[idx] : shifts[idx + 1] if idx + 1 < len(shifts) else len(inp)]
    for idx in range(len(shifts))
]

shift_dict = defaultdict(int)
for shift in split_shifts:
    if len(shift) > 1:
        shift_dict[int(guard_nb_pat.search(shift[0]).group(1))] += reduce(
            lambda x, y: y - x,
            [int(minutes_pat.search(item).group(1)) for item in shift[1:]],
        )

most_asleep_guard_id = max(shift_dict, key=shift_dict.get)

midnight_hour_dict = {k: 0 for k in range(60)}
for shift in split_shifts:
    if "#" + str(most_asleep_guard_id) in shift[0] and len(shift) > 1:
        for i in range(1, len(shift) - 1, 2):
            for minute in range(
                int(minutes_pat.search(shift[i]).group(1)),
                int(minutes_pat.search(shift[i + 1]).group(1)),
            ):
                midnight_hour_dict[minute] += 1

print(most_asleep_guard_id * max(midnight_hour_dict, key=midnight_hour_dict.get))


# part 2
guards_minutes_dict = {guard_id: {k: 0 for k in range(60)} for guard_id in shift_dict}
for shift in split_shifts:
    if len(shift) > 1:
        for minute in range(
            int(minutes_pat.search(shift[i]).group(1)),
            ...
            guards_minutes_dict[int(guard_nb_pat.search(shift[0]).group(1))][
                minute
            ] += 1

guard = max(
    guards_minutes_dict,
    key=lambda guard_id: max(guards_minutes_dict[guard_id].values()),
)
print(guard * max(guards_minutes_dict[guard], key=guards_minutes_dict[guard].get))
