...
MANDATORY_KEYS = [elem +':' for elem in ('byr','iyr','eyr','hgt','hcl','ecl','pid')]

def is_valid(passport:str) -> bool:
    return all(elem in passport for elem in MANDATORY_KEYS)
...
byr_pat = re.compile(r'byr:(\d{4})\D')
iyr_pat = re.compile(r'iyr:(\d{4})\D')
eyr_pat = re.compile(r'eyr:(\d{4})\D')
hcl_pat = re.compile(r'hcl:#[0-9a-f]{6}[^0-9a-f]')
ecl_pat = re.compile(r'ecl:(amb|blu|brn|gry|grn|hzl|oth)\s')
pid_pat = re.compile(r'pid:\d{9}\D')
hgt_pat = re.compile(r'hgt:((\d{3})cm|(\d{2})in)\s')

patterns = [byr_pat, iyr_pat, eyr_pat, hcl_pat, ecl_pat, pid_pat, hgt_pat]

def is_valid_part2(passport:str)->bool:
    if all(pat.search(passport) for pat in patterns):
        ...
        if byr in range(1920, 2003):
            ...
            if iyr in range(2010, 2021):
                ...
                if eyr in range(2020, 2031):
                    # print('pass eyr')
                    hgt = hgt_pat.search(passport)
                    if ('cm' in hgt.group() and int(hgt.group(2)) in range(150,194)) or ('in' in hgt.group() and int(hgt.group(3)) in range(59,77)):
                        return True
    return False
...
print(byr_pat.search('byr:19290'))