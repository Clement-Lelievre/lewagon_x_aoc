...
nb_range_pat = re.compile(r"^(\d+)-(\d+)")


def is_valid(row: str) -> bool:
    """Says whether word is valid as per the `Official Toboggan Corporate Policy`
    ...
    """
    m, M = (s := nb_range_pat.search(row)).group(1), s.group(2)
    range_ = range(int(m), int(M) + 1)
    letter = (rs := row.split())[1][0]
    return rs[-1].count(letter) in range_
