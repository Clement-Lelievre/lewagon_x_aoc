...
def make_monkey_dict(input_: str) -> None:
    ...
    lines = [row for row in input_.splitlines() if row.strip()]
    for line in lines:
        monkey_name, *rest = pat.findall(line)[0]
        monkey_dict[monkey_name] = (
            int(elem)
            if (elem := rest[0]).isdigit()
            else {"left": rest[2], "op": op_dict[rest[3]], "right": rest[4]}
        )
...
make_monkey_dict(INPUT)
print("part 1:", find_root_value())
...
# see notebook for my solution

# I used the above make_monkey_dict function to create a dict with the humn value as key, and reused from part 1
# the function find_root_value to find the root value for each humn value
# I did all this using a binary search
