...
INPUT = """dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
# An IP supports TLS if it has an Autonomous Bridge Bypass Annotation, or ABBA. An ABBA is any four-character sequence which consists of
# a pair of two different characters followed by the reverse of that pair, such as xyyx or abba. However, the IP also must not have an ABBA
# within any hypernet sequences, which are contained by square brackets.

inp = [elem.replace("\n", "") for elem in INPUT.splitlines()]

outside_brackets_pat = re.compile(r"(\w+)(?:\[\w+\])?")
inside_brackets_pat = re.compile(r"\[(\w+)\]")


def abba_in(part: str) -> bool:
...
def supports_tls(ip: str) -> bool:
    inside_brackets = inside_brackets_pat.findall(ip)
    outside_brackets = outside_brackets_pat.findall(ip)
    return any(abba_in(part) for part in outside_brackets) and all(
        not abba_in(part) for part in inside_brackets
    )


# print(sum(supports_tls(ip) for ip in inp))
...
aba_pat = re.compile(
    r"(([a-z])[^\2]\2)", re.IGNORECASE
)  # or (\w)(?!\1)\w\1 using negative lookahead
# interestingly, with the Python REGEX engine this fails to capture all relevant groups
# on other engines the 2 above REGEX patterns work. So I'll go for a more manual solution


def find_abas(seq: str) -> list[str]:
    """finds and returns all ABA patterns found

    Args:
        seq (str): the 'supernet' sequence

    Returns:
        list[str]: the list of ABA-like subsequences
    """
    abas = []
...
def reverse_aba(aba: str) -> bool:
    """Reverses and returns the aba passed, as per the instructions above
    Exemple: reverse_aba("xyx") = "yxy"
    """
...
def supports_ssl(ip: str) -> bool:
    ...
    outside_brackets = outside_brackets_pat.findall(ip)
    abas = []
    for supernet_seq in outside_brackets:
        abas += find_abas(supernet_seq)
    return any(
        reverse_aba(aba) in hypernet_seq
        for aba in abas
        for hypernet_seq in inside_brackets
    )


print(sum(supports_ssl(ip) for ip in inp))
