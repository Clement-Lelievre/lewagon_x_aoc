...
# no test input for part 1!
nodes = []
for row in INPUT.splitlines():
    s = pat.search(row)
    if s is not None:
        node = tuple(map(int, (s.group(1), s.group(2), s.group(3), s.group(4))))
        nodes.append(node)
nb_viable_pairs = 0
for node1, node2 in combinations(nodes, 2):
    if node1[3] >= node2[2] > 0:
        nb_viable_pairs += 1
    if node2[3] >= node1[2] > 0:
        nb_viable_pairs += 1

print(f'{nb_viable_pairs=}')

# part 2
target_x, target_y, target_used, target_avail = max(nodes, key=lambda x: x[0])

graph = {
    node: frozenset(
        cand for cand in nodes if abs(cand[0] - node[0]) + abs(cand[1] - node[1]) == 1
    )
    for node in nodes
}  # frozenset is not mandatory but the intent is clear: immutable, unordered and unique
