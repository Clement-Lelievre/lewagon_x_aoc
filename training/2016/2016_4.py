...
checksum_pat = re.compile(r'\[(.*?)\]$')
encrypted_name_pat = re.compile(r'[\D+-]+')
sector_pat = re.compile(r'-(\d+)\[')

def real_room_sector(name: str) -> int:
    encrypted_name = encrypted_name_pat.match(name).group().replace('-','')
    checksum = checksum_pat.search(name).group(1)
    l = [(letter, encrypted_name.count(letter)) for letter in set(encrypted_name)]
    #print(''.join(elem[0] for elem in sorted(l, key = lambda x : (-x[1], x[0]))[:5]))
    if ''.join(elem[0] for elem in sorted(l, key = lambda x : (-x[1], x[0]))[:5]) == checksum:
        return int(sector_pat.search(name).group(1))
    return 0

nb = 0

for row in INPUT.split():
    nb += real_room_sector(row)
print(nb)
...
real_names = []
def decipher(name: str):
    sector = int(sector_pat.search(name).group(1))
    encrypted_name = encrypted_name_pat.match(name).group()

for row in INPUT.split():
    if real_room_sector(row):
        encrypted_name = encrypted_name_pat.match(row).group().replace('-',' ')
        shift = int(sector_pat.search(row).group(1))
        real_name = ''.join(ascii_lowercase[(ascii_lowercase.index(letter)+shift)%26] if letter != ' ' else letter for letter in encrypted_name)
        if re.search(r'north\s*pole',real_name, re.IGNORECASE):
            print(row)
