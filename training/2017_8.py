import re
from operator import le, lt, gt, ge, ne, eq, add, sub

INPUT = """utc dec -736 if p > -7
tn inc -876 if qlm == 4
uz dec 294 if l < 10
a inc -904 if me >= -7
tn inc 622 if ppl <= 6
...
inp = [e for e in INPUT.split("\n") if e]
pat = re.compile(
    r"^(\w+)\s(dec|inc)\s(-?\d+)\sif\s(\w+)\s(==|!=|>|<|<=|>=)\s(-?\d+)", re.IGNORECASE
)


def extract_instructions(instr: str) -> list[str]:
    """Parses instruction and returns its key elements

    Args:
        instr (str): the instruction

    Raises:
        Exception: if the REGEX pattern is unable to parse the instruction

    Returns:
        list[str]: the list of instruction features
    """
    if (s := pat.search(instr)) is None:
        raise Exception(f"Cannot parse: {instr}")
    return [s.group(i) for i in range(1, 7)]


registers = set() # get a unique list of registers
instruction_features = []
for item in inp:
    r = extract_instructions(item)
    registers.add(r[0])
    registers.add(r[3])
    instruction_features.append(r)

registers_dict = {register: 0 for register in registers}  # initiate registers values
operator_dict = {
    "inc": add,
    "dec": sub,
    "<": lt,
...
for features in instruction_features:
    ...
    if operator_dict[condition_operator](
        registers_dict[register_condition], int(condition_value)
    ):
