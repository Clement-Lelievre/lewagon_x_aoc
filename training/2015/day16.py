...
for i in range(len(inp)):
    d[i + 1] = {k: int(v) for k, v in pat.findall(inp[i])}
...
for i in d:
    for k in d[i]:
        if (
            ...
            and d[i][k] != real_sue[k]
        ):
            break
        elif k in ("pomeranians", "goldfish") and d[i][k] > real_sue[k]:
            break
        elif k in ("cats", "trees") and d[i][k] < real_sue[k]:
            break
    else:
        candidates.append((i, d[i]))
