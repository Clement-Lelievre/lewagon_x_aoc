...
# print(instructions)
grid = [[0]*1_000 for i in range(1_000)]


# part 1
def turn_off(instruction, grid):
    row_start = int(''.join([c for c in instruction[:instruction.index(',')] if c.isdigit()]))
    column_start = int(''.join([c for c in instruction[instruction.index(','):instruction.index('through')] if c.isdigit()]))
    instruction = instruction[instruction.index('through'):]
    row_end = int(''.join([c for c in instruction[:instruction.index(',')] if c.isdigit()]))
    column_end = int(''.join([c for c in instruction[instruction.index(','):] if c.isdigit()]))
...
#         grid = turn_on(instruction, grid)
#     elif 'turn off' in instruction:
#         grid = turn_off(instruction, grid)
#     elif 'toggle' in instruction:
#         grid = toggle(instruction, grid)
...
def turn_off(instruction, grid):
    ...
    for row in range(row_start, row_end+1):
        for column in range(column_start, column_end+1):
            grid[row][column] = max(0, grid[row][column] - 1 ) 
    return grid
