...
"""
instructions = [item for item in instructions.split('\n') if item != '']
rows = []
valid_rows = []
for item in instructions:
    if "shiny gold" in item[(ind := item.find('bags')):]: # the walrus operator!
        rows.append(item[:ind].strip())
...
while True:  
    ...
    for bag in rows:
        for item in instructions:
            if bag in item[(ind := item.find('bags')):]: # in the right-hand side
                l.append(item[:ind].strip()) # append the left-hand side
                valid_rows.append(item)
                answer += 1
    rows = l
    if not l:
        break
print(len(set(valid_rows)))
