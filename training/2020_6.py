...
for i in range(len(inp)):
    if (bit := inp[i]):
        ...
        groups.append(group_data.replace(' ',''))
        group_data = ''
...
for i in range(len(inp)):
    if (bit := inp[i]):
        group_data.append(bit)
    else:
        groups.append(group_data)
        group_data = []
groups.append(group_data)

def nb_questions(group: list[str]) -> int:
    count = 0
    for letter in ascii_lowercase:
        for member in group:
            if not letter in member:
                break
        else:
            count += 1
    return count

print(sum(nb_questions(group) for group in groups))