...
ld = {k:len(v) for k,v in digit_dict.items()}
d = defaultdict(list)
for length in set(ld.values()):
   for k in ld.keys():
       if ld[k] == length:
           d[length].append(k)
#print(d)
# input = [item[item.find('|')+1:].strip() for item in input.split('\n') if item != '']
# output = 0
...
# part 2
inputs = [item[:item.find('|'):].strip().split(' ') for item in input]
outputs = [item[item.find('|')+1:].strip().split(' ') for item in input]
input_dict = list(zip(inputs, outputs))

def len_identify(output: list[str]) -> bool | int:
    for word in output:
        if not len(word) in (2,3,4,7):
            return False
    return int(''.join([str(d[len(word)][0]) for word in output]))

answer = 0  
for item in input_dict:
    start, end = item[0], item[1]
    start = [''.join(sorted(word)) for word in start]
    end = [''.join(sorted(word)) for word in end]
    decipher_dict = defaultdict(str)
    if (a := len_identify(end)): # easy case: guess digit based on words length
        answer += a
    else:
    # identify 1,4,7,8 based on their unique length 
        one =   [word for word in start if len(word) == 2][0]
        four =  [word for word in start if len(word) == 4][0] 
        seven = [word for word in start if len(word) == 3][0]  
        eight = [word for word in start if len(word) == 7][0]
        decipher_dict[one] = 1
        decipher_dict[four] = 4
        decipher_dict[seven] = 7
        decipher_dict[eight] = 8
        words_to_guess = [word for word in start if word not in decipher_dict.keys()]
        for word in words_to_guess: # main case
            if (len(word) == 5) and len([letter for letter in seven if letter in word]) == 3:
                decipher_dict[word] = 3
            elif (len(word) == 6) and len([letter for letter in set(seven+four) if letter in word]) == 5: # put this condition above the next one else logic will fail
                decipher_dict[word] = 9
            elif (len(word) == 6) and len([letter for letter in seven if letter in word]) == 3:
                decipher_dict[word] = 0
            elif (len(word) == 6) and len([letter for letter in set(seven+four) if letter in word]) != 5:
                decipher_dict[word] = 6
            elif (len(word) == 5) and len([letter for letter in four if letter in word]) == 2:
                decipher_dict[word] = 2
            else:
                decipher_dict[word] = 5
        answer += int(''.join([str(decipher_dict[word]) for word in end])) # there could be an issue with first digit is 0, but it never happened in my input apparently as it raised no error whatsoever
print(answer)
